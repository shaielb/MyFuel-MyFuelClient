...
public class StationSupplyOrdersScreen extends SceneBase {
	...
	public void initialize() throws Exception {
		...
		//scene switchers
		_mainMenuStationManagerScreenControl = new MfImageView((ImageView) _scene.lookup("#scene$MainMenuSupplierScreen"));
		_mainMenuStationManagerScreenControl.addEvent((event) -> { _switcher.switchScene("MainMenuSupplierScreen"); });
		...
		_supplyOrderConfirmationPopScreenControl.addEvent((event) -> {
			...
			if (!Enums.InProcess.equals(sso.getOrderStatus())) {
				UiHandler.showAlert(AlertType.ERROR, "Station Supply Order", "", "Can Only Select 'In Process' Requests");
			}
			else {
				IUpdate updateRequest = _client.getUpdateRequest();
				sso.setOrderStatus(Enums.Done);
				updateRequest.addEntity(sso);
				try {
					ResponseEvent re = _client.sendRequest(updateRequest);
					re.continueWith(response -> {
						if (!response.isPassed()) {
							UiHandler.showAlert(AlertType.ERROR, "Station Supply Order", "", response.getDescription());
						}
						else {
							UiHandler.showAlert(AlertType.INFORMATION, "Station Supply Order", "", "Selected Station Supply Order Is Done");
							...
							try {
								...
								responseEvent.continueWith((responseAfterDeletion) -> {
									if (!responseAfterDeletion.isPassed()) {
										UiHandler.showAlert(AlertType.ERROR, "Orders Transaction", "", responseAfterDeletion.getDescription());
									}
									else {
							...
							sf.setStation(sso.getStation());
							QueryContainer qc = new QueryContainer(sf);
							qc.addQueryCondition("station_fk", "=");
							filterQuery = _client.getFilterRequest();
							filterQuery.addQueryContainer(qc);
							try {
								ResponseEvent responseEvent = _client.sendRequest(filterQuery);
								responseEvent.continueWith((resonseInFilter) -> {
									if (!resonseInFilter.isPassed()) {
										UiHandler.showAlert(AlertType.ERROR, "Orders Transaction", "", resonseInFilter.getDescription());
									}
									else {
										Collection<StationsFuel> entities = resonseInFilter.getEntitiesAsType();
										for (StationsFuel sf1: entities) {
											sf1.setCurrentAmount(sf1.getCurrentAmount() + sso.getAmount());
											IUpdate updateRequest1 = _client.getUpdateRequest();
											updateRequest1.addEntity(sf1);
											try {
												ResponseEvent re1 = _client.sendRequest(updateRequest1);
												re1.continueWith(response1 -> {
													if (!response.isPassed()) {
														UiHandler.showAlert(AlertType.ERROR, "Station Supply Order", "", response.getDescription());
													}
												});
